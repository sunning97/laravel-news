	...
	<div class="visible-xs col-xs-12">
		<div class="mobile_hot_img">
			<a href="{{url('/chi-tiet/'.$item->slug)}}" title="{{$item->newsname}}">
				<img src="{{ asset('img/news/300x300/'.$item->newimg)}} " alt="{{$item->newimg}}">
			</a>
		</div>
...
@elseif($i<20)
	<div class="visible-xs col-xs-6 padding4">
		<div class="mobile_img">
			<a href="{{url('/chi-tiet/'.$item->slug)}}" title="{{$item->newsname}}">
				<img src="{{ asset('img/news/300x300/'.$item->newimg)}}" alt="{{$item->newsname}}">
			</a>
		</div>
		<div class="mobile_title">
			<a href="{{url('/chi-tiet/'.$item->slug)}}" title="{{$item->newsname}}"><h1>{{$item->newsname}}</h1></a>
...
@else
	<div class="item">
		<div class="item-header">
			<a href="{{url('chi-tiet/'.$item->slug)}}" title="{{$item->newsname}}">
				<b>{{$count}}</b>  <img src="{{ asset('img/news/300x300/'.$item->newimg)}} " alt="no img" width="50" />
			</a>
		</div>
		<div class="item-content">
		...
		<div class="item">
			<div class="item-header">
				<a href="{{url('chi-tiet/'.$item->slug)}}">
					<img src="{{ asset('img/news/100x100/'.$item->newimg)}}" alt="no img" width="80" />
				</a>
			</div>
			<div class="item-content">
